...
$students = $conn->query("
    SELECT 
        ...
        DATEDIFF(CURDATE(), s.enrollment_date) as days_elapsed,
        -- Calculate total course days
        (s.duration_months * 30) as total_course_days,
        -- Calculate expected end date
        DATE_ADD(s.enrollment_date, INTERVAL s.duration_months MONTH) as expected_end_date,
        -- Calculate days remaining
        DATEDIFF(DATE_ADD(s.enrollment_date, INTERVAL s.duration_months MONTH), CURDATE()) as days_remaining,
        -- Calculate completion percentage
        ROUND((DATEDIFF(CURDATE(), s.enrollment_date) / (s.duration_months * 30)) * 100, 2) as completion_percentage
    FROM students s 
